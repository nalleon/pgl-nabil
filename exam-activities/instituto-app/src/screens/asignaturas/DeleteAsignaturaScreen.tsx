...
const DeleteAsignaturaScreen = (props: Props) => {
  ...
  useEffect(() => {
      const fetchData = async () => {
          ...
          try {
              const response = await axios.get(`${URL_INSTITUTO}v3/asignaturas`, {
                  headers: {
  ...
  function showConfirmation (nombre : string, curso : string){
    Alert.alert(`Eliminar`,`¿Está seguro de que desea borrar la  ${nombre} cuyo curso es ${curso}?`,
      [
        {
          text: 'No',
...
export default DeleteAsignaturaScreen
